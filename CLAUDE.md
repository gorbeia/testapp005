...
2. **Lessons (`LESSONS`)** — derived automatically from `VERBS` as the cross product of (verb × tense), in data order. A lesson is identified by `${verbId}-${tense}`. Adding a new verb or tense to `VERBS` automatically produces new lessons; no separate lesson registry to maintain.
