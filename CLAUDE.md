...
The entire app currently lives in `src/App.jsx` as a single file, organized top-to-bottom into clearly delimited sections (look for the `===` banner comments):
...
2. **Lessons (`LESSONS`)** — derived automatically from `VERBS`, ordered simple-and-singular → richer-and-combined. *Practice* lessons are the (verb × tense) cross product, in data order (`{ id: '${verbId}-${tense}', verbId, tense }`) — one conjugation at a time. Once a verb has more than one tense, a *review* lesson combining all of them is appended right after (`{ id: '${verbId}-review', review: true, sources: [{ verbId, tense }, …] }`); once there's more than one verb, a final cross-verb "mixed review" caps the whole sequence. Adding a new verb or tense to `VERBS` automatically produces the right new practice and review lessons; no separate lesson registry to maintain. `describeLesson` centralises the practice-vs-review display copy (`LessonNode`/`ProgressTab`/`LessonResultsScreen` all use it) and `groupLessonsByVerb` decides whether a lesson belongs in its verb's section or — for cross-verb reviews — a trailing "Review" section.
